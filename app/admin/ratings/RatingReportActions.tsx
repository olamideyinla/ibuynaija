...
export default function RatingReportActions({ reportId }: { reportId: string }) {
  ...
  async function doAction(action: string) {
    setError('');
    setLoading(action);
    const res = await fetch(`/api/admin/ratings/${reportId}`, {
      method: 'POST',
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        >
          {loading === 'resolve' ? 'Resolving…' : 'Mark resolved (keep rating)'}
        </button>
        <button
          onClick={() => doAction('hide_rating')}
          disabled={loading !== null}
        ...
        >
          {loading === 'hide_rating' ? 'Hiding…' : 'Hide rating'}
        </button>
      </div>
    </div>
  );
